---
title: "Pause for thought Experiment 2 - data analysis"
author: "Zhang Chen"
...
df_main <- df_main %>%
  mutate(
    ...
    choose_high_EV = ifelse(game2_choice == high_EV_option, 
                            "yes", "no"),
...
The experimental trials often have the same structure. In half of the experimental pairs, the high-probability option has higher expected values; in the remaining half, the low-probability option has higher expected values. The differences in EV are much smaller though in the experimental trials than in the catch trials. Next I compute and plot the probability of picking the option with a higher expected value (@fig-high-ev-choice).
...
df_main %>%
  ...
  group_by(subject_ID, high_EV_option) %>%
  summarize(prop_high_EV = mean(choose_high_EV == "yes") * 100) %>%
  ggplot(aes(subject_ID, prop_high_EV)) +
  geom_line(aes(group = 1), color = "gray") +
  geom_point() +
  labs(x = "Participant", 
       y = "The proportion of choosing high EV options") +
...
# # fit a brms model - go for the maximum random structure
# brms_file_name <- 
#   paste("brms-fits/fit_EV_diff", 
#         high_prob_cutoff, "LP", low_prob_cutoff, sep = "_")
# 
# fit_HP_choice <- brm(
#   game2_choose_HP ~ EV_diff * game1_outcome_num * delay_num +
#     (EV_diff * game1_outcome_num * delay_num|subject_ID),
#   family = bernoulli(link = "logit"),
#   prior = 
#     c(
#       prior(normal(0, 2), class = Intercept),
...
robustness_plot <- overall_summary_selected %>%
  ...
  labs(x = "Number of high EV choices on LP-optimal trials",
       y = "Estimate (log odds-ratio)",
       color = "Number of high EV choices on HP-optimal trials") 

ggsave("plots/robustness_Exp2.png", robustness_plot,
       width = 7, height = 9)
...
### Prospect theory analyses

Prospect theory posits that people transform objective value and probability into subjective utility and likelihood, and choose based on the perceived value rather than the objective expected value. In this analysis, I follow the analyses done by Sharp et al.[@sharp2012]. I use the same equations and parameters in their analyses (see their [Supplemental Materials](https://journals.plos.org/plosone/article?id=10.1371/journal.pone.0033460#s5)), and repeat the above analyses using EU ratio (Expected Utility ratio) rather than EV ratio.

```{r}
df_exp <- df_exp %>%
  mutate(
    # Sharp et al. used values from 1 to 5 (coins),
    # whereas I used 10 till 50 cents. Divide by 10 to be consistent.
    # 0.57 is the parameter they used for this value function.
    HP_utility = (game2_HP_amount/10) ** 0.57,
    LP_utility = (game2_LP_amount/10) ** 0.57,
    # the same function and parameter used by Sharp et al.
    # for weighting the probability of winning
    HP_likelihood = exp(-(-log(game2_HP_prob))**0.77),
    LP_likelihood = exp(-(-log(game2_LP_prob))**0.77),
    # compute the perceived value
    HP_EU = HP_utility * HP_likelihood,
    LP_EU = LP_utility * LP_likelihood,
    # compute the EU ratio on each trial
    EU_ratio = (HP_EU - LP_EU)/(HP_EU + LP_EU) * 2
  )

# fit the model again, this time using EU ratio rather than EV ratio
fit_prospect_theory <- brm(
  game2_choose_HP ~ EU_ratio * game1_outcome_num * delay_num +
    (EU_ratio * game1_outcome_num * delay_num|subject_ID),
  family = bernoulli(link = "logit"),
  prior = 
    c(
      prior(normal(0, 2), class = Intercept),
  ...
  warmup = 2000,
  seed = 1234,
  file = "brms-fits/fit_prospect_theory",
  backend = "cmdstanr"
)
```

```{r}
#| label: fig-prospect-ppc-general
#| fig-cap: General posterior predictive checks for the prospect theory model.
#| fig-subcap: 
#| - "As a function of delay"
#| - "As a function of game 1 outcome"
#| - "As a function of EV ratio"
...
# model diagnostics
# check the trace plot, looks good.
# plot(fit_prospect_theory)

# posterior predictive checks
yrep <- posterior_predict(fit_prospect_theory)
...
ppc_bars_grouped(df_exp$game2_choose_HP, yrep, 
                 df_exp$EU_ratio, prob = 0.95, freq = FALSE)
```

Posterior predictive checks show that the fit seems to have improved for the effect of EU ratio, although there are still some deviations between the observed and predicted data. Note that in this analysis I simply use the parameters of Prospect Theory from previous work (same as Sharp et al.), but these parameters of course vary across individuals, and may also vary across conditions (e.g. delay vs. no delay).

```{r}
tab_model(fit_prospect_theory)
```

The results remain largely the same. Participants are again more sensitive to EU ratio after a delay, although this time the 95% CI for the odds ratio just includes 0.
...
df_startRT <- df_after %>%
  mutate(
    # one trial has a start RT of 0, set it to 1 ms
    game2_startRT = ifelse(game2_startRT == 0, 1, game2_startRT),
    log_startRT = log(game2_startRT)
  )
...
```{r, results='asis'}
tab_model(fit_choiceRT_student)
```
...
# save as a png file
ggsave("plots/RTs_Exp2.png", RT_plot, width = 4, height = 4)

# also save as a rds file for later use
saveRDS(RT_plot, "plots/RTs_Exp2.rds")
```
