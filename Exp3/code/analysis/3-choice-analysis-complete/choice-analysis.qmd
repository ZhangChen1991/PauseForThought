---
title: "Experiment 3 - choice analysis (exploratory dataset)"
author: 
  - name: "Zhang Chen"
...
For this set of analyses, I use the complete exploratory data set. More concretely, participants were **not** excluded based on their performance on LP-optimal catch trials, as in other experiments in this project.
...
df_main <- read_csv("../../../data/processed/df_main_exp3_complete.csv")
```
...
write_csv(df_exp_selected, 
          "../../../data/processed/df_exp_exp3_complete.csv")
...
# save as a png file
ggsave(filename = "../plots/EV_delay_Exp3_complete.png", 
       EV_delay_plot,
       width = 4, height = 4)
```
...
saveRDS(choice_pause_outcome_plot, "../plots/choice_plot_exp3.rds")
```
...
# save as a png file
ggsave(filename = "../5-plots/prob_amount_Exp3_complete.png", 
       prob_amount_post_plot,
       width = 7, height = 8)
```
...
df_n_subj <- overall_summary_selected %>%
  ...
  mutate(LP_cutoff = as.factor(LP_cutoff))

# plot the posterior distributions from all models
robustness_plot <- overall_summary_selected %>%
  mutate(LP_cutoff = as.factor(LP_cutoff)) %>%
  ggplot(aes(LP_cutoff, Estimate)) +
  geom_hline(yintercept = 0, linetype = "dashed") +
  geom_point() +
  geom_errorbar(aes(ymin = CI_lower, ymax = CI_upper),width = 0.2,) +
  # add the number of participants left
  geom_text(data = df_n_subj, aes(LP_cutoff, 1, label = n_subj)) +
  facet_wrap(~param_label, ncol = 1, scales = "free_y") +
  labs(x = "Number of LP choices on LP-optimal catch trials",
       y = "Estimate (log odds-ratio)") 

robustness_plot

ggsave("../plots/robustness_Exp3.png", robustness_plot,
       width = 6, height = 8)
```
...
# code Experiment 2 as 0.5, and Experiment 3 as -0.5
