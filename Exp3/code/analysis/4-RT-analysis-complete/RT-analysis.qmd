---
title: "Experiment 3 - RT analysis (complete dataset)"
author: 
  - name: "Zhang Chen"
...
df_exp <- 
  read_csv("../../../data/processed/df_exp_exp3_complete.csv")
```
...
# save the figures
ggsave("../plots/startRT_Exp3_complete.png", startRT_plot, width = 2.5, height = 5)
ggsave("../plots/startRT_effs_Exp3_complete.png", startRT_effs_plot, width = 8, height = 6)
```
...
# save the plot
ggsave("../plots/ppc_median_startRT_Exp3.png", width = 6, height = 5)
```
...
# save the figures
ggsave("../plots/choiceRT_Exp3_complete.png", choiceRT_plot, width = 2.5, height = 5)
ggsave("../plots/choiceRT_effs_Exp3_complete.png", choiceRT_effs_plot, width = 8, height = 6)
```
...
# save the plot
ggsave("../plots/ppc_median_choiceRT_Exp3.png", width = 6, height = 5)
```
...
Compare the start and choice RTs of the current experiment with those of Experiment 2.
...
df_exp3 <- read_csv("../../../data/processed/df_exp_exp3_complete.csv")
...
```{r}
# prepare data for start RT analysis
df_startRT_exp2_exp3 <- df_exp2_exp3 %>%
  filter(game2_startRT <= 5000) %>%
  mutate(log_startRT = log(game2_startRT + 1))

# fit a brms model, with student's t as the likelihood function
fit_startRT_exp2_exp3 <- brm(
  log_startRT|trunc(ub = log(5001)) ~  
  ...
  data = df_startRT_exp2_exp3,
  cores = 4,
  iter = 7000,
  warmup = 2000,
  seed = 1234,
  refresh = 10,
  file = "brms-fits/fit_startRT_exp2_exp3",
  backend = "cmdstanr"
  )
```
...
```{r}
# prepare data for choice RT analysis
df_choiceRT_exp2_exp3 <- df_exp2_exp3 %>%
  filter(game2_respRT <= 5000) %>%
  mutate(log_choiceRT = log(game2_respRT + 1))

# fit a brms model, with student's t as the likelihood function
fit_choiceRT_exp2_exp3 <- brm(
  log_choiceRT|trunc(ub = log(5001)) ~  
    game1_outcome_num * delay_num * Exp_num +
    (game1_outcome_num * delay_num|subject_ID),
  family = student(),
  ...
  data = df_choiceRT_exp2_exp3,
  cores = 4,
  iter = 8000,
  warmup = 3000,
  seed = 1234,
  refresh = 10,
  file = "brms-fits/fit_choiceRT_exp2_exp3",
  backend = "cmdstanr"
  )
```
...
```{r}
# fit a brms model, with student's t as the likelihood function
fit_choiceRT_exp2_exp3_baseline <- brm(
  log_choiceRT|trunc(ub = log(5001)) ~ Exp_num + (1|subject_ID),
  family = student(),
  prior = 
    c(
      prior(normal(6.5, 1), class = Intercept),
  ...
  seed = 1234,
  refresh = 10,
  file = "brms-fits/fit_choiceRT_exp2_exp3_baseline",
  backend = "cmdstanr"
  )
```
